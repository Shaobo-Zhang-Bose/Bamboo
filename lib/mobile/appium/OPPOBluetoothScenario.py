...
from wham_automation.test_scripts.Bamboo.lib.mobile.constants import OPPOConstant
from wham_automation.lib.mobile.scenario.PhoneInfo import PhoneInfo


class OPPOBluetoothScenario(BasePage, BluetoothInterface):
    def __init__(self, android_locators=OPPOConstant,
    ...
    def bt_enabled(self):
        ...
        :raises:None
        """
        # logger.info("hello from bt_enabled")
        is_bt_enabled = False
        try:
            ...
            if toggle is not None:
                # is_enabled() method does not apply here
    ...
    def launch_settings(self, driver=None):
        ...
        try:
            ...
            bluetooth_xpath = '//android.widget.TextView[@text="Bluetooth"]'

            self.wait_till_element_to_be_visible(self.driver.appium_driver, self.bluetooth_tab)
            self.find_element(self.driver.appium_driver, self.bluetooth_tab).click()
    ...
    def bt_radio(self, enable='on'):
        ...
        for i in range(5):
            if target_status == current_status:
                ...
                bt_toggle = self.find_element(self.driver.appium_driver, self.bluetooth_button_on_off_button, 0)
                if bt_toggle is not None:
                    bt_toggle.click()
    ...
    def handle_pairing_window(self, wait_time=5):
        ...
        try:
            ...
            permission_checkbox = self.find_element(self.driver.appium_driver, self.do_not_ask_again_checkbox, 0)
            if permission_checkbox is not None:
                permission_checkbox.click()
            else:
                logger.info("There is no permission checkbox.")
            ok_pair_button = self.find_element(self.driver.appium_driver,
                              self.bluetooth_pair_button, 0)
            if ok_pair_button is not None:
                ok_pair_button.click()
            else:
                logger.info("Cannot find OK/Pair button.")
    ...
    def is_same_bluetooth_name(self, device_to_connect,
        ...
        logger.info("Current device is %s, dut device is %s" % (name_of_bluetooth, bluetooth_device_to_connect))
    ...
    def connect_paired_device(self, target):
        self.wait_till_element_to_be_visible(self.driver.appium_driver,
                                             self.paired_device_text, 10)
        paired_device_elements = self.find_elements(self.driver.appium_driver,
                                                  self.paired_device_text,
                                                  0)
        for index in range(len(paired_device_elements)):
            logger.info("examing {}".format(paired_device_elements[index].text))
            if self.is_same_bluetooth_name(paired_device_elements[index],
                                           target):
                logger.info("Found target device, now connecting...")
                paired_device_elements[index].click()
                self.handle_pairing_window()
                self.handle_pairing_window()
                return True
    ...
    def _select_bluetooth_device(self, bluetooth_device_name_to_connect,
        ...
        try:
            ...
            while not is_device_found:
                ...
                if attempt_times % 5 == 0:
                    logger.info("failed to connect with {}".format(
                        bluetooth_device_name_to_connect))
                    fail_cnt += 1
                    ...
                    if fail_cnt == 10:
                        ...
                        logger.info("Failed to find target for %d time. Try Again." % fail_cnt)
                ...
                try:
                    element_list = self.find_elements(self.driver.appium_driver,
                                                      self.bluetooth_pair_device_with_title)
                except Exception as e:
                    logger.info("Failed to get all available devices, try again")
                    continue

                device_list_length = len(element_list)

                if device_list_length < 3:
                    continue
                else:
                    for index in range(3, device_list_length):
                        ...
                        try:
                            ...
                            if match_dut_name:
                                ...
                                logger.info("Found dut device")
                                logger.info("Clicking DUT device to connect")
    ...
    def wait_find_and_click_element_by_xpath(self, element_xpath, wait_time=5):
        """
        ...
        """
        try:
            WebDriverWait(self.driver.appium_driver, wait_time, 1).until(lambda x: x.find_element_by_xpath(element_xpath))
        except Exception as e:
            logger.exception("exception in wait_find_and_click")
            return False
        else:
            logger.info("found it, now click it!")
            try:
                self.driver.appium_driver.find_element_by_xpath(element_xpath).click()
                wait(5)
            except Exception as e:
                logger.exception(e)
                return False
